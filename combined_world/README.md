...
| Controller | Role |
|------------|------|
| **`coordinator`** | C — **Supervisor** TCP server (**`REGISTER …`**, **`CMD`**, **`TELEM`**, UI **`CONTROL *`**). **UR phase timing:** `ceil(8 / UR_SPEED_MULT)` coordinator steps between grasp / release dwell edges, **never fewer than 8** so the Robotiq can close before **`ROTATING_ARM`**. **`SCARA_SPEED_DIV`:** `scara_coordinator_step()` every **N** supervisor steps (default **1**, matches Webots sample **`example_code/scara_food_industry.py`** cadence). TCP sends use **retrying writes** so non-blocking sockets do not drop **`CMD`** / SCARA lines. Optional **`SIM_SLOWDOWN`**. |
| **`ur_arm_executor`** | C — one process per UR; **TCP client** to the coordinator (same port); sends **`TELEM`** each step; applies **`CMD …`** lines. **`controllerArgs`**: `[ speed, UR3e | UR5e | UR10e ]`. Grasp/release motor targets match **`example_code/ure_can_grasper.c`**: finger **0.85**, release to each finger’s **min**; **`wb_motor_set_velocity`** on the **four arm joints only** (not finger motors). **`GRASPING_CAN`** / **`RELEASING_CAN`** are **re-applied each step** while active (TCP scheduling). Same **`SIM_SLOWDOWN`**. |
...
| **`scara_food_industry`** | Python — **TCP client**; **`SCARA_*`** mirrors **`example_code/scara_food_industry.py`**. **Merge:** `VACCUM.getPosition()` is **world**; fruit `translation` is **parent-local** (SCARA island is under `Pose +12 m`), so merge converts with **`R^T (p_world − p_parent)`** before `setSFVec3f`. Poll TCP **before** `supervisor.step`. **`SIM_SLOWDOWN`** default **1**. |
